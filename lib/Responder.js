...
module.exports = class Responder {
    ...
    async awaitNumberedResponse(content, [start, end]) {
        ...
        const responses = await this.messages.channel.awaitMessages(msg => msg.author.id === this.message.author.id && parseInt(msg.content) && parseInt(msg.content) >= start &&
...
const EMOJIS = {
    success: '<:animatedcheck:488337189831442432>',
    error: '<:animatedcross:488337189831442432>',
    ok: '<:blobthumbs:488337189831442432>',
}
